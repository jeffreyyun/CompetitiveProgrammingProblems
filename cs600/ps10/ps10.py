...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    for k in kvals:
        error = 0
        holdout_err = 0
        clusters = kmeans(training, k, cutoff, County)[0]
        for c in clusters:
            for p in c.getPoints():
                error += p.distance(c.getCentroid())**2
        for p in holdout:
            dist = None
            for c in clusters:
                if dist == None or p.distance(c.getCentroid()) < dist:
                    dist = p.distance(c.getCentroid())
            holdout_err += dist**2

        tt_err.append(error)
        th_err.append(holdout_err)  
        t_ratio.append(error/holdout_err)

    pylab.subplot(1,2,1)
    pylab.plot(kvals, tt_err, 'b-', label="training err")
    pylab.plot(kvals, th_err, 'r-', label="holdout err")
    pylab.legend(loc="upper right")
    pylab.subplot(1,2,2)
    pylab.plot(kvals, t_ratio, 'g-', label="ratio")
    pylab.show()      

#graphRemovedErr(testPoints)
# clusters = kmeans(points, 50, 0.1, County)[0]
# for c in clusters:
#     if c.contains('CALosAngeles'):
#         for p in c.getPoints():
#             print p.getName()
        ...
        for p in holdout:
            ...
            for c in clusters:
                if dist == None or p.distance(c.getCentroid()) < dist:
                    dist = p.distance(c.getCentroid())
                    clust = c
            for p in clust.getPoints():
                c_pov += p.getOriginalAttrs()[3]
            c_pov /= len(clust.getPoints())
            pov_err += (p.getOriginalAttrs()[3]-c_pov)**2;

        t_pov_err.append(pov_err)

    pylab.plot(kvals, t_pov_err, 'b-', label="training err")
    pylab.legend(loc="upper right")
    pylab.show()      

graphPredictionErr(points, 3)